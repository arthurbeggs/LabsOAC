...
#Ataque do pokemon inimigo#
animationOpponentPokemonAtack:
    
    animationOpponentAtack:
    addi $sp, $sp, -20
    sw $a0, 0($sp)
    sw $a1, 4($sp)
    sw $a2, 8($sp)
    ...
    la $t9, background
    li $s0, 0x30242088
animationOpponentAtack.nextPixelX:
    addi $sp, $sp, -4
    sw $ra, 0($sp)
    jal drawEnemyPokemonHP
    lw $ra, 0($sp)
    ...
    addiu $s0, $s0, -4   # Proxima Coluna
    li $t0, 0x30244C30  # Foram todas as colunas
    bne $s0, $t0, animationOpponentAtack.nextPixelX# necessarias?

animationOpponentAtack.previousPixelX:
#    cleanAreaPokemon $t9    # "Limpa" tela  
    addi $sp, $sp, 4
    sw $ra, 0($sp)
    jal drawEnemyPokemonHP
    ...
    jal cleanAreaPokemon
    addiu $s0, $s0, 4  	# Volta um Coluna
    li $t0, 0x30242074		# Foram todas as colunas
    bne $s0, $t0, animationOpponentAtack.previousPixelX# necessarias?
end.animationOpponentAtack:
    lw $a0, 0($sp)
    lw $a1, 4($sp)
    lw $a2, 8($sp)
    lw $a3, 12($sp)
