...
endException:	lw $ra,4($sp)
		...
		mfc0 $k0,$14    #### EPC + 4     #nao esta implementada no pipe
		addi $k0,$k0,4
...
syscallException: addi $sp, $sp, -256   			# Salva todos os registradores na pilha
	sw $1,0($sp)
	sw $2,4($sp)
	sw $3,8($sp)
	sw $4,12($sp)
...
li $t0, 0x41200000 #10
mtc1 $t0, $f24
c.lt.s $f10, $f23 #vÃª se mantissa Ã© menor que 1
bc1f NOTZERO
mul.s $f10, $f24, $f10
...
mtc1 $t0, $f24
c.lt.s $f10, $f24 #vÃª se mantissa Ã© menor que 1
bc1t NOTZERO
div.s $f10, $f10, $f24
addi $s4, $s4, +1 #decrementa o expoente por 1
j normalizaPraBaixo
...
stepE1: addi $s1, $s1, 1
	...
	beq $t0, '+', stepE2
	bne $t0, '-', stepEE	# '-' = 45 Se for negativo prossegue, se nï¿½o ir para loop1
...
n10:	addi $sp,$sp,-20  #Argumento esta em t7 = n, saida em 10**n = $f31#Argumento esta em t7 = n, saida em 10**n = $f31
	sw $ra,0($sp)
	sw $t0,4($sp)
	sw $t7,8($sp)
	swc1 $f0,12($sp)
